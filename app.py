...
# ================= SESSION STATE =================
for key, default in [("clauses", None), ("analysis", None), ("history", []), ("theme", "light"), ("chat", []), ("chat_open", False)]:
...
def generate_summary_text():
    ...
    summary = f"""CONTRACT ANALYSIS REPORT
...
if page == "Overview":
    ...
    st.markdown(f"<div class='subtitle'>Automated risk assessment powered by advanced AI</div>", unsafe_allow_html=True)
    ...
    if st.button("Run Analysis", type="primary", use_container_width=True):
        with st.spinner("Analyzing contract clauses..."):
            ...
            st.session_state.analysis = analyze_contract(st.session_state.clauses)
    ...
    st.markdown(f"""<div class='card'>
        ...
        <div style='color: {text}; line-height: 2;'>
            ...
            <p><strong>Recommendation:</strong> <span style='color: {"#ef4444" if high > 0 else success}; font-weight: 600;'>{"Legal review strongly advised" if high > 0 else "Contract appears acceptable"}</span></p>
        </div>
    </div>""", unsafe_allow_html=True)
